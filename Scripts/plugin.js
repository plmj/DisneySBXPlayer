...
ContentDirect.UI.Storefront.HTML.prototype = {
    ...
    // Actual embedding once data is ready
    appendPlayer: function (data, mediaItem, performHDCPCheck) {
        ...
        jQuery.ajax({
            ...
            if (nativePlayback) {
                ...
                if (legacyHtmlPlayer) {  // player version is 15.2.HTPL
                ...
                else {  // We're NOT using legacy player
                    ...
                    uiController.initialize().then(function () {                        
                        ...
                        StorefrontScriptAPI.EventOccured("Error", "FailedInitializingHtmlPlayer", errorMessage);                        
								...
								ascendon.htmlplayer.events.on('htmlplayer.updatecontentprogresssuccess', function(event, data){
									//alert('hey');
									console.log(data);
								}); 
								ascendon.htmlplayer.events.on('htmlplayer.updatecontentprogresserror', function(event, data){
									//alert('hey!');
									console.log(data);
								}); 
								// Build audio list
                ascendon.htmlplayer.events.on('htmlplayer.readytoplaycompleted', function () {
                		var tempHtml = '';
                		$('.asc-player-media-title').text(data.ContentName.split('-')[0]);
                    for(var i = 0; i < data.AvailableContentItems.length; i++) {
                    	var current = data.AvailableContentItems[i];
                    	var id = current.Id;
                    	var activeId = JSON.parse(decodeURIComponent(StorefrontInternalAPI._initParams.viewproductcontentjson)).ContentItemId;
                    	var description = current.Description;
                    	if(id == activeId){
                    		tempHtml += '<li class="activeAudio asc-player-text-track" val="' + id +'">' + description +'</li>';
                    	}
                    	else {
                    		tempHtml += '<li class="asc-player-audio-track" val="' + id +'">' + description +'</li>';
                    	}
                    	
                    }
                    $('.asc-player-captions-audio').append('<div class="asc-player-audioSection"><h1>Audio</h1><ul>' + tempHtml + '</ul></div>');
                    $(document).on('click','.asc-player-audioSection ul li', function(){
                    	var id = $(this).attr('val');
											playerController.viewContent(id);
                    });
